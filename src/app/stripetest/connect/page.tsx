...
export default function ConnectPage() {
    ...
    useEffect(() => {
        ...
        let cancelled = false
        fetch(`/api/connect/account-status?accountId=${encodeURIComponent(accountId)}`, {
            credentials: 'include',
        })
            .then((r) => r.json())
            .then((data) => {
    ...
    return (
        <div className="mx-auto max-w-2xl space-y-6 px-4 py-8">
            ...
            {accountId == null && (
                <section className="rounded-lg border bg-card p-4">
                    <p className="mb-2 text-sm text-muted-foreground">
                        ​支払いを受け付けるには、まず Connect アカウントを作成してください。
                    </p>
                    <Button
            ...
            {accountId != null && (
                <>
                    <section className="rounded-lg border bg-card p-4">
                        <h2 className="mb-2 font-medium">オンボーディング状態</h2>
                        {status == null ? (
                            ...
                            <ul className="mb-4 list-inside list-disc text-sm">
                                <li>
                                    支払い準備完了:{' '}
                                    {status.readyToProcessPayments ? 'はい' : 'いいえ'}
                                </li>
                                <li>
                                    オンボーディング完了:{' '}
                                    {status.onboardingComplete ? 'はい' : 'いいえ'}
                                </li>
                        ...
                        >
                            {linkLoading ? 'リンク取得中…' : '支払いを受け付けるためにオンボードする'}
                        </Button>
                    </section>
                    ...
                    <section className="rounded-lg border bg-card p-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            <Button asChild variant="outline" size="sm">
                                <a href={`/connect/products?accountId=${encodeURIComponent(accountId)}`}>
                                    商品を追加
                                </a>
                            </Button>
                            <Button asChild variant="outline" size="sm">
                    ...
                    <section className="rounded-lg border bg-card p-4">
                        <h2 className="mb-2 font-medium">プラットフォームサブスクリプション</h2>
                        <p className="mb-2 text-sm text-muted-foreground">
