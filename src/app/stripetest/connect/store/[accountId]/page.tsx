...
export default function ConnectStorePage() {
    ...
    useEffect(() => {
        if (!accountId) return
        fetch(`/api/connect/products?accountId=${encodeURIComponent(accountId)}`)
            .then((r) => r.json())
            .then((data) => {
    ...
    const getPriceId = (p: Product): string | null => {
        ...
        if (def && typeof def === 'object' && 'id' in def) return (def as Price).id
        return null
    }
    ...
    if (!accountId) {
        return (
            <div className="mx-auto max-w-2xl px-4 py-8">
                <p className="text-muted-foreground">ストア ID がありません。</p>
            </div>
        )
    }
            ...
            {products.length === 0 ? (
                ...
                <ul className="grid gap-4 sm:grid-cols-2">
                    {products.map((p) => {
                        ...
                        const price =
                            p.default_price && typeof p.default_price === 'object' && p.default_price
                        ...
                        return (
                            ...
                            >
                                ...
                                <Button
                                    className="mt-auto"
                                    size="sm"
                                    disabled={!priceId || checkoutLoading !== null}
                                    onClick={() => handleBuy(p)}
                                >
                                    {checkoutLoading === p.id ? 'リダイレクト中…' : '購入する'}
                                </Button>
                            </li>
                        )
                    })}
